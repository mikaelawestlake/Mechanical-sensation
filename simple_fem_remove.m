function [SED] = simple_fem_remove(nelx, nely, forces, applied, x, y, shape, psi_bar)

% Function calculates the strain energy density at each element within the
% lattice based on the geometry and applied load. 
...
% Remove elements for damaged nodes in lattice 
if strcmp(shape,'vert') == 1
struc(x-1:x+1,y)=0;
elseif strcmp(shape,'horz') == 1
struc(x, y-1:y+1)=0;    
end 
...
xlim([1 11])
ylim([1 31])
xlabel(' j' , 'FontSize', 12, 'FontName', 'times')
ylabel('k', 'FontSize', 12, 'FontName', 'times')
set(gca, 'YDir','reverse')
yticks([1 6 11 16 21 26 31] )
%print(gcf, sprintf('SED outer vert - %f .png', applied),'-dpng','-r300') 
